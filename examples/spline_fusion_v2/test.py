...
class Spline(object):
    ...
    def weights(t):
        u = t-numpy.floor(t)
        u2 = u*u 
        u = numpy.c_[numpy.ones_like(u), u, u2, u*u2]
        return u.dot(Spline._C.T)         # nx4
    
    def trans(self, xyz, local_ts, return_se3=False): 
        xyz = xyz.reshape(-1,3)
        u = Spline.weights(local_ts)             # nx4
...
def _prepare_data():
    ...
    return {
        ...
        'ref_w':    Spline.weights(ref_ts),
        'src_w':    Spline.weights(src_ts),
    }

def _dump2csv(fname, arr):
    m,n = arr.shape if arr.ndim==2 else (arr.shape[0],1)
    arr = arr.reshape(m,n)
    with open(fname, mode='w') as fh:
        for i in xrange(n):     # col major
            val = map(str, arr[:,i])
            val = ','.join(val)
            fh.write('%s\n'%val)
    print '... saved %s'%fname
...
if __name__=='__main__':
    ...
    if 0:
        ...
        for k,v in _data.items():
            if not isinstance(v, numpy.ndarray): continue
            _dump2csv(_fname%k, v)
