...
UniformSpline<double> create_zero_spline(double* data, size_t num_knots) {
    UniformSpline<double> spline(1.0, 0.0);
    //const size_t num_knots = 10;
    spline.zero_knots(data, num_knots);
    return spline;
}
...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<double> ptr(new double[10*UniformSpline<double>::param_size]);
    UniformSpline<double> dummy_spline = create_zero_spline(ptr.get(), 10);
    ...
    cout << "\n\n------------------------- NUMERIC ------------------------------------" << endl;
    UniformSpline<double> spline = create_zero_spline(ptr.get(), 10);
